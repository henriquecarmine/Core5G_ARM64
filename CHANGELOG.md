...
| 0.98.0 | 2026-09-12 | **O SMO também na topologia do Projeto 1, e o SMO de pé depois de religar a instância.** A topologia do P1 (Open5GS + UERANSIM) ganha o que o P2 tem desde a 0.91.0: as bandas SMO e "rede gerenciada por O1", os 10 contêineres com cartão completo, os links (HTTPS, Proxy, RESTCONF, OAuth, O1, M-plane, VES, Kafka) e as interfaces na legenda, as camadas 6 e 7 do tour e as etapas 6 (o que é o SMO e como se liga à rede do P1), 6.1 (O1) e 6.2 (VES) da jornada, os logs do controlador, do VES, do gateway e da O-DU simulada, e o botão SMO na régua de serviços do P1. No P1 o acesso do operador é desenhado como ele é hoje, pelo Caddy (`caddy → smo-gateway`), e a coluna do SMO foi posicionada pelo verificador de geometria até ficar limpa. Os cartões do SMO passam a ter um texto só para as duas topologias (`topo.smo.*`), com os dois trechos que citavam o P2 reescritos para servir a ambos (nenhum rádio do lab tem O1; o Non-RT RIC ainda não está ligado ao SMO). Os textos novos do P1 (bandas, tour, jornada e dica do botão) estão nos 4 idiomas. O minimapa dos 7 testes do SMO passa a desenhar o projeto em foco, em vez de sempre o P2. O teste da topologia agora exige o SMO nas duas: bandas, interfaces, 10 cartões completos, tour com 7 camadas no P1 e jornada com 16 etapas, a 6 explicando o SMO no P1. **Religamento:** a instância foi desligada à 01:01 e religada às 08:12; o SMO parecia no ar (controlador "healthy"), mas o RESTCONF pelo gateway não respondia e nenhum elemento conectava. O Traefik passou a usar o endereço da rede `smo` dos serviços, onde o gateway não estava; `docker network connect smo gateway` resolveu na hora (elementos de volta em 10 s, `test_smo.sh` OK), e o override do common põe o gateway nas três redes. Documentado no README do SMO |
| 0.97.5 | 2026-09-12 | **Console do SMO no modo escuro.** O ODLUX do upstream só tem o tema claro, com as cores fixas no build. Agora o nginx do próprio contêiner injeta na página o `server/smo/odlux/core5g-tema.css`, que só vale com o aparelho no escuro: fundo, cartões, barra, menu, abas, tabelas, paginação e campos, mirando as classes globais do MUI v5 (medidas na página antes de escrever). Os gráficos do Home são canvas, e o texto preto no centro das roscas sumia; eles têm as cores invertidas por filtro, com a matiz girada para o verde continuar verde. Os ícones escuros do menu foram clareados. Quem aplica é `smo_tema_odlux` (`lib.sh`), chamado pelo `up_smo.sh`: reescreve um bloco entre marcadores no `location.rules`, e o nginx valida antes de recarregar (se recusar, o bloco sai). No claro, nada muda. A tradução do ODLUX segue fora: seria editar o pacote compilado. O painel não mudou |
| 0.97.4 | 2026-09-12 | **Login do SMO nos 4 idiomas, claro e escuro.** A tela de login do Keycloak passa a falar português (padrão), inglês, espanhol e francês, com seletor de idioma, e usa o tema `keycloak.v2` do próprio Keycloak 26.1.2, que acompanha o modo claro ou escuro do aparelho (o tema `oam` do upstream só tem o claro). As traduções são as que o Keycloak já traz; o `smo_acesso_web.sh` aplica tudo no realm, então sobrevive a uma nova subida do SMO. Conferido na internet nas 8 combinações (4 idiomas × claro/escuro), com login real do operador. O console ODLUX em si segue só em inglês e claro: o build do upstream não tem tradução nem modo escuro, e traduzir significaria editar o pacote compilado. O painel não mudou |
| 0.97.3 | 2026-09-12 | **Entrar no console do SMO sem cair no formulário errado.** A tela do ODLUX desenha o formulário local (Username / Password / Domain) antes de carregar o botão ONAP-IDENTITY; quem digitava ali recebia "Could not log in. Please check your credentials…" — o formulário é bloqueado pelo Caddy (emitia token para o admin do controlador) e nem conhece os usuários do Keycloak, que não registrou nenhuma tentativa. Agora a raiz do console (`https://odlux.oam.smo.<host>/`) redireciona direto para o login do Keycloak (`/oauth/login/identity`, o mesmo caminho do botão), e com sessão aberta volta direto ao console. `/odlux/index.html` continua abrindo, e o formulário local segue bloqueado. Conferido com login real pelos dois caminhos (raiz e botão): entra como o operador e a aba Connect lista a O-DU e o O-RU. Limitação registrada no README do SMO, e que já existia antes: o canal de notificações ao vivo do console (`/websocket`) é recusado pelo controlador com o OAuth ligado, porque o ODLUX abre o WebSocket sem credencial; as telas carregam normalmente, só as notificações em tempo real não chegam. O painel não mudou |
| 0.97.2 | 2026-09-12 | **Login do console do SMO igual ao do painel.** O operador do ODLUX passa a ser o mesmo usuário do painel, com a mesma senha, guardados em `server/smo/.odlux-acesso` no servidor e no `.env` local (fora do git); o `martin.skorupski` do upstream ficou desativado. O `smo_acesso_web.sh` cria o operador no realm quando ele ainda não existe, com o papel `administration` e o perfil preenchido (o Keycloak não pede atualização no primeiro login), e continua deixando só ele ativo. Achado: cada comando do `kcadm` sobe uma JVM e a execução leva uns 2 min com o servidor carregado; uma rodada que parecia morta pelo timeout do ssh seguia viva e as duas apagavam a configuração uma da outra ("No server specified") — agora cada execução usa a sua (`mktemp`). Conferido com login real no ODLUX: entra como o operador novo e a aba Connect lista a O-DU e o O-RU. O painel não mudou |
| 0.97.1 | 2026-09-12 | **Botão SMO no cabeçalho.** Ao lado do "mapa", em roxo para não se confundir com ele: a bolinha fica verde com o SMO no ar e cinza desligado (o mesmo estado do botão SMO da barra de serviços) e o clique abre o SMO ao vivo. O modal ganhou o link **Console ODLUX ↗**, que abre o console numa aba nova; o endereço sai do próprio host do painel (`odlux.oam.smo.<host>`, a mesma regra do `smo.env`), sem domínio escrito na página. Ligar e desligar continua só na barra de serviços. Textos nos 4 idiomas; o teste visual confere que o botão abre o modal com o link |
| 0.97.0 | 2026-09-11 | **Aulas da cadeira de Gestão e Orquestração (SMO), e três correções do painel.** A cadeira 5 entra nos Estudos com as duas aulas do Prof. Lucas Borges, lidas dos PDFs agora em `pdfs/04-gestao-orquestracao/`: **Aula 0 — Revisão** (13 conceitos, 11 perguntas) e **Aula 1 — SMO** (17 conceitos, 1 fórmula, 8 perguntas), nos 4 idiomas. As perguntas trazem a resposta do próprio professor, tirada do slide seguinte (ele repete a pergunta com a resposta marcada, às vezes só pela cor, conferida na imagem); as abertas dizem que a leitura é da aula, não dele; três inconsistências dos slides (10, 51 e 62) ficam apontadas no texto. Cada aula liga aos testes reais do SMO. O rail da cadeira 5 ganhou o link "📖 Aulas desta cadeira" (o grupo estava com `data-estudo="e4"`, copiado da 4), o catálogo a bancada "SMO", e os índices em inglês, espanhol e francês os rótulos dos 7 testes do SMO, que faltavam desde a 0.93.0. As rotas `/lab/estudo/N` e `/aula/K` leem do catálogo quais cadeiras e quantas aulas existem (eram 1–4 e 1–6 fixos). **Bug latente:** a lista de aulas linkava pelo número do professor e a rota abre pela posição — com a Aula 0, "Aula 1" abriria a Aula 0; o link usa a posição e anterior/próxima mostram o número do professor. Novo `test:estudos`: índices e aulas batem entre si nos 4 idiomas (ids, números, cenas do minimapa, comandos com rótulo, quiz com pergunta e resposta). **Contêineres travados:** o "containers ▾" da telemetria não abria nem fechava — a explicação no clique dos instrumentos capturava o `<summary>` e mostrava um cartão vazio; controles de verdade (summary, botões, links e a própria lista) ficam fora dela, e o teste visual confere abrir, fechar e os instrumentos seguirem explicando. **Mapas dos testes legíveis:** ligações, nomes das interfaces, bordas e nomes dos componentes fora do fluxo, bandas e linhagem com mais contraste e letras maiores (a janela mostra o mapa a 60–80%). **Bolinhas por onde o dado passa:** um trem de pacotes percorre o caminho inteiro, trecho por trecho, na ordem dos dados, com dois trens meio ciclo defasados e bolinhas maiores; ao fim do teste elas seguem andando na cor do resultado em vez de sumir. Quem pede menos movimento vê as bolinhas paradas no meio de cada trecho |
| 0.96.0 | 2026-09-11 | **O console do SMO direto no navegador, sem túnel.** O ODLUX abre em `https://odlux.oam.smo.core5g-arm64.duckdns.org`, com certificado válido e login do Keycloak em `identity.smo.core5g-arm64.duckdns.org`. O SMO trocou o domínio de exemplo do upstream (`smo.o-ran-sc.org`) por um subdomínio do servidor (o DuckDNS resolve qualquer nível), e o Caddy do painel publica só esses dois nomes; o Traefik continua em 127.0.0.1:8443. Antes de ir para a internet, o SMO foi fechado por `server/smo/smo_acesso_web.sh` (idempotente): o realm tinha **auto-cadastro aberto**, "esqueci a senha" sem e-mail, nenhuma proteção contra força bruta e seis usuários ativos — cinco com a senha padrão pública e um criado pelo script do upstream. Agora cadastro e recuperação estão desligados, a força bruta protegida e só um operador fica ativo, com senha forte gerada no servidor e guardada fora do git. O Caddy recusa Basic auth (o nginx do ODLUX repassa ao controlador tudo o que não é arquivo, e o RESTCONF aceitava a senha padrão do admin), o **formulário local do ODLUX** (`POST /oauth/login` emitia token para o admin do controlador com a senha padrão — confirmado pela internet antes do bloqueio), o console de administração e o realm master do Keycloak; kafka-ui, painel do Traefik e VES seguem só por dentro. O controlador continua indo ao Keycloak pelo gateway (`trustAll`); o kafka-ui, cuja descoberta OIDC confere o nome do certificado, passou a ir pelo Caddy. Achados da troca, documentados no README do SMO: o arquivo de OAuth do controlador é montado com o domínio fixo (e montagem de arquivo não aceita `sed -i`), o Traefik servia o certificado genérico para os nomes novos, o Caddy precisava repassar o Host, o `up_smo.sh` morria calado sob `pipefail` quando não havia mais o que trocar, e o controlador recriado escolhe outro `controllerId` — o ODLUX entrava, mas listava zero elementos até o banco ser realinhado ao id atual (`smo_alinha_controlador`). Na topologia, o cartão do gateway, a jornada e o tour deixaram de falar em túnel SSH, nos 4 idiomas |
| 0.95.1 | 2026-09-11 | **SMO ao vivo legível na projeção.** No cabeçalho do modal, o "auto (10 s)" quebrava em três linhas; no barramento Kafka, os nomes longos de tópico empurravam o último evento para a linha de baixo no meio da frase. Agora o cabeçalho não quebra e o último evento de cada tópico tem a própria linha, recuada, embaixo do tópico e da contagem |
| 0.95.0 | 2026-09-11 | **SMO ao vivo: o retrato do SMO dentro do painel, sem túnel.** O botão "SMO ao vivo (o que o operador vê)" na cadeira 5 e o endereço curto **/smo** abrem um modal de texto no estilo dos logs, só leitura e atualizando a cada 10 s: **elementos sob gerência** (a aba Connect do ODLUX: O-DU e O-RU híbrido, estado NETCONF, endereço da sessão e modelos YANG O-RAN/3GPP), **conexões recentes** (connectionlog do controlador, na hora do servidor), **alarmes ativos** e **histórico** (faultcurrent/faultlog), **barramento Kafka** (eventos por tópico e o último de cada um, com a idade) e os **15 contêineres** das três camadas. Quem lê é `server/smo/smo_ao_vivo.sh`, com o mesmo acesso dos testes da apresentação (gateway local, `lib.sh`): o último evento de todos os tópicos sai de um único consumidor na ponte do Kafka e a leitura inteira leva ~3 s; `/api/smo` guarda 8 s de cache, então a turma toda custa uma leitura. Por que não o ODLUX de verdade dentro do painel: ODLUX e Keycloak mandam `X-Frame-Options: SAMEORIGIN` e `frame-ancestors 'self'`, o login usa endereços fixos sem porta e as senhas são as padrão públicas do upstream; o ODLUX continua pelo túnel SSH (`server/smo/README.md`, agora com os usuários e a troca de senha do primeiro login). SMO desligado vira instrução, não tela vazia. Textos nos 4 idiomas; o teste visual confere as 6 seções, os estados coloridos, a idade do último evento, o SMO desligado e o Escape |
| 0.94.0 | 2026-09-11 | **O rótulo "servidor" da telemetria abre as características do servidor.** Clique (ou Enter/Espaço) no rótulo abre um modal com tudo o que o host informa, lido na hora por `/api/server-info` (cache de 15 s): **instância** (metadados EC2 por IMDSv2: tipo, id, ciclo de vida, AMI, região, zona, nomes e IPs), **processador** (`lscpu`: modelo com a geração do Graviton — Neoverse-N1 = Graviton2 —, arquitetura, vCPU, topologia, caches, flags), **memória** (`/proc/meminfo`, swap, swappiness), **armazenamento** (discos do `lsblk` e partições com total/usado/livre), **sistema** (SO, kernel, fuso, NTP, tempo ligado, carga), **rede** (interfaces com endereço, estado e MTU), **Docker** (versão, driver, contêineres, e o espaço de imagens/volumes/cache de build) e **painel** (versão, Python, serviços core5g-panel/caddy/docker). O `docker system df` leva dezenas de segundos com 41 imagens, então roda em segundo plano (renovado a cada 5 min) e a coleta fica abaixo de 1 s. Rótulos nos 4 idiomas; valores técnicos como o servidor informa. Achado: a "explicação no clique" dos instrumentos capturava qualquer clique em elemento com `title` na telemetria e engolia o do rótulo — quem tem `role="button"` agora faz a própria ação. O teste visual confere clique e teclado, as 8 seções, nenhuma chave de tradução crua e a formatação de bytes, partição, duração e ✓ |
| 0.93.1 | 2026-09-11 | **A jornada apresenta o SMO antes de mostrá-lo trabalhando.** Nova etapa **7 — "SMO: o que é, para que serve e como se liga à nossa rede"**: a camada de gestão e orquestração da O-RAN (o RIC decide sobre o rádio; o SMO opera a rede), rodando no mesmo servidor ARM64 em redes Docker só dele, sem exposição à internet (operador → gateway por túnel SSH → console → controlador), gerenciando a O-DU e os O-RU simulados porque o gNB do OAI não tem O1 — o Core 5G não passa por ele e o Non-RT RIC ainda não está integrado. O desenho acende o caminho usuário → gateway → ODLUX → controlador → O-DU, com zoom automático. As etapas de gestão viram 7.1 (O1) e 7.2 (VES); a jornada passa a 20 etapas. A camada 8 do tour ganhou a mesma frase de ligação. Textos nos 4 idiomas; o teste visual confere que a etapa explica o que é o SMO e como ele se liga ao lab |
| 0.93.0 | 2026-09-11 | **A apresentação de SMO em funções reais: um teste do painel por item da avaliação do Prof. Lucas Borges.** Cadeira 5 no rail ("Gestão, Orquestração e Automação (SMO)") com 7 testes que falam com o SMO no ar pelo gateway, e o botão **SMO** na barra de serviços do Projeto 2 (liga/desliga a pilha, status pelo contêiner do controlador). **Arquitetura** (itens 1–3): as 3 camadas e os 15 contêineres arm64 lidos do Docker, os 8 serviços das rotas do gateway e os elementos O-RAN sob gerência, dizendo o que fica fora do OAM (A1, O2). **O1: leitura** (4–5): RESTCONF → NETCONF na O-DU, 154 modelos YANG, a árvore 3GPP ManagedElement → GNBDUFunction → NRCellDU. **O1: provisionamento** (5): PATCH no nome da gNB-DU, conferido no datastore dentro do elemento e desfeito (com desfazer garantido se algo falhar). **Falhas** (8): alarme linkDown CRITICAL por VES até o tópico de falhas do Kafka em ~1 s, e a limpeza. **Telemetria** (8): o arquivo 3GPP TS 28.532 da O-DU, o aviso FileReady no Kafka e os contadores de heartbeat/registro. **Ciclo de vida** (7): encerra e reinstancia o O-RU; o SMO percebe a queda em ~1 s e o call home remonta em ~10 s, com o connectionlog do controlador. **O-Cloud e O2** (4 e 6): inventário do O-Cloud do lab e a lacuna do O2, que no O-RAN SC é do projeto INF. Cada teste tem explicação no console, cena na faixa de execução com cartão por etapa, cena no minimapa (com zoom na coluna do SMO) e rótulos nos 4 idiomas. O Kafka é lido pelo **kafka-bridge** do próprio SMO (HTTP): pelo console do Kafka cada leitura subia uma JVM e o teste de falhas levava 45 s — agora 3 s. Achados da montagem: a geração de medidas do simulador de O-DU travou uma vez (reiniciar o contêiner resolve; o teste avisa quando o arquivo está velho) e, com o E2 lab do P2 ligado, o gNB e o UE simulados ocupam ~3 dos 4 núcleos e o controlador chegou a não responder em 25 s. Roteiro de 20 min em `docs/apresentacao-smo.md` |
| 0.92.0 | 2026-09-11 | **Zoom automático na apresentação e nos processos.** Com a rede maior (o SMO à direita), o mapa inteiro deixava os cartões pequenos demais. Na **topologia**, cada camada do tour e cada etapa da jornada aproximam sozinhas dos seus nós, com animação curta (sem animação para quem pede menos movimento), enquadrando na proporção do palco e descontando a parte coberta pela caixa da legenda; nunca aproxima além de 980 unidades, então uma etapa de um nó só ainda mostra os vizinhos; ao sair, volta ao mapa inteiro. O botão ⌖ junto do zoom liga/desliga (lembrado no navegador; dica nos 4 idiomas); roda e arrasto manuais interrompem a animação. No **minimapa** dos testes (a janela do console), o mapa enquadra os componentes acesos e a linhagem do dado; "mapa todo" / "foco" alterna, e a cor do resultado (verde/vermelho) sobrevive ao redesenho. O teste visual confere: zoom forte numa etapa compacta, enquadramento completo numa etapa alta, botão desligado mantendo o mapa inteiro, retorno ao sair, e o minimapa enquadrando, alternando e preservando o resultado |
| 0.91.0 | 2026-09-11 | **O SMO entra na topologia do Projeto 2, com tudo o que os outros grupos têm.** Duas bandas novas à direita: **SMO — O-RAN SC OAM** (gateway/Traefik, Keycloak, topologia NTSim-NG, ODLUX, controlador SDN-R, coletor VES e Kafka, na cor de gestão) e **Rede gerenciada por O1** (O-DU e os dois O-RU simulados, na cor da RAN). Cada contêiner tem o cartão completo — de onde vem, o que faz, para onde vai, imagem ARM64, portas, rede e conexões — com o que o `test_smo.sh` mediu no servidor (154 capacidades YANG na O-DU, 114 no O-RU híbrido, o hierárquico que só a O-DU gerencia). 12 links com os caminhos reais: O1 e M-plane (NETCONF), VES, Kafka, RESTCONF, OAuth e os proxies do gateway. **Tour** com 2 camadas a mais (9), **Jornada** com 2 etapas opcionais (7 · gestão pela O1 e 7.1 · eventos do VES ao Kafka, 19 no total) e o resumo final citando a gestão; modo **Fluxo** anima O1/M-plane/VES/Kafka; **Troubleshooting** e o status ao vivo usam os nomes dos contêineres do SMO; **Logs** ganham controlador, VES, gateway e O-DU; a tela de **Análise** agrupa os contêineres como "SMO · O-RAN SC OAM". Textos nos 4 idiomas (224 chaves novas) e 10 verbetes novos no glossário (NETCONF, YANG, RESTCONF, M-plane, PNF, TAPI, OAuth, Kafka, Keycloak, OpenDaylight), com balão no desenho e nas legendas. O canvas passou de 1300 para 1800 de largura; a geometria segue limpa (nenhum link atravessa cartão). O teste visual agora confere as 7 bandas, as interfaces do SMO no desenho, o tour de 9 camadas com legenda em todas, os 10 cartões completos, a jornada de 19 etapas e os papéis em francês |
| 0.90.0 | 2026-09-11 | **SMO do O-RAN SC (OAM) rodando em ARM64 no servidor, para a cadeira nova de Gestão, Orquestração e Automação (Prof. Lucas Borges).** Em `server/smo/`, a solução docker compose do repositório `o-ran-sc/oam` com as três camadas: *common* (Traefik, Keycloak, MariaDB, Kafka, topologia), *oam* (controlador SDN-R/OpenDaylight da O1, ODLUX, coletor VES) e *network* (simuladores pynts de O-DU e O-RU). As imagens do O-RAN SC para o SMO só existem em amd64 (conferido com `skopeo`); `build_arm64.sh` resolve as 7: controlador, ODLUX e VES por **troca de base** (a aplicação Java ou estática é copiada da oficial para uma base aarch64, sem executar nada amd64) e topologia e simuladores **compilados da fonte** em C (libyang, sysrepo, netopeer2, ntsim-ng). O servidor já usa 80/443 (Caddy do painel), então o gateway publica só em `127.0.0.1:8443` e ganha IPs fixos nas redes `dcn`/`dmz`, por onde simuladores, controlador e kafka-ui o alcançam no lugar do IP do host — tudo em `smo.env` e `compose/*.override.yaml`, sem editar o upstream. Quatro problemas apareceram na montagem, **nenhum de ARM64**: o JDK 11 atual valida o campo ZIP64 dos jars e o Karaf da topologia não subia; o Traefik v3.3.6 do upstream fixa a API 1.24 do Docker, que o Docker 29 recusa, e ficava sem rotas (trocado pelo v3.6.25); o script de realm do Keycloak exige `USER`; e a primeira troca de base do ODLUX esqueceu dois scripts de `sbin/` (conferido depois com `docker export` da oficial). `test_smo.sh` confere pelo gateway: realm do Keycloak, controlador pronto, **O-DU e O-RU conectados por NETCONF/TLS** (call home), heartbeat aceito pelo VES e publicado no Kafka, registro de PNF dos simuladores e medidas 3GPP no barramento. Uso de memória ~5 GB; disco do servidor ampliado de 30 para 80 GB. Senhas padrão do upstream: nada sai do localhost |
| 0.89.0 | 2026-09-11 | **Cenários sugeridos pelo servidor nos testes de dados: 1, 2, 5 ou 10 celulares × distância × interferência, com a explicação minuciosa na tela.** O cartão 💾 dos 7 temas, do `p2-kpi-qoe` e do `p2-closed-loop` ganhou "cenário gerado pelo servidor": celulares (1/2/5/10), distância (100 m, 500 m, 1 km, 3 km ou misturadas) e interferência (nenhuma, fraca, média, alta, numa janela no meio da carga). O gerador `temas/cenarios_kpm.py` usa **a mesma física do Lab do UE** (perda de percurso 3GPP TR 38.901 UMa NLOS a 3,5 GHz, C/I, SINR, Shannon), calibrada na amostra real do professor — 1 celular a 100 m sem interferência reproduz a amostra —, e marca cada linha como **sintética**. Antes do resultado, cada teste imprime as 7 fórmulas, a tabela por celular com vazão **esperada × observada**, o efeito na célula e "como ler este teste neste cenário" (no closed loop: quantos celulares já ficam abaixo dos 8 Mbit/s da política, onde o rate-limit não cortaria nada). Com vários celulares, as regras que olham o tempo passam a rodar sobre a célula (uma linha por instante) — na lista crua a janela misturaria celulares do mesmo instante. **Com a amostra real nada mudou**: as 10 saídas foram gravadas antes e comparadas depois, idênticas. 11 testes novos no `test:tudo`; o cartão testado no Chrome com a API simulada. De passagem: a cena do closed loop da 0.88 não tinha as informações dos nós, então o cartão de dados não abria nela; e o próprio cartão 💾, anexado fora do console, ficava **ilegível no tema claro** (texto do console, claro, sobre fundo claro) — agora carrega a superfície escura nos dois temas |
| 0.88.0 | 2026-09-11 | **Teste novo das aulas 05 e 06: `p2-closed-loop`, o closed loop A1 passo a passo, sem a RAN.** Os 8 passos do roteiro da demo (slide 23 da aula 06) sobre a mesma telemetria dos 7 temas: baseline calmo e MAD → `decision.json` (regra do `ai_policy_pipeline.py` do professor: maioria da janela de 5) → policy A1 em dry-run → `action_request.json` (rate_limit 8 Mbit/s na `oaitun_ue1`, com os IDs que amarram decisão, política e ação) → `effect_report.json` com média, mediana, variação relativa e pontos percentuais → rollback, terminando no SEHAL e gravando os artefatos no formato do lab do professor. O `tc` e o envio ao PMS são **impressos, nunca executados** (o script nem importa `subprocess`). Lendo o modo offline do professor para portar, apareceu o que o teste agora se recusa a repetir: o `effect_report` da amostra dele mostra a vazão caindo 78 mil kbps, mas o "depois" é a fase de recuperação do mesmo experimento e nenhum rate-limit rodou — o nosso relatório sai com `"causal": false` e a tela diz que **não é efeito da atuação**, com os números da execução ao vivo dos slides como referência. No painel: botão no rail da cadeira 4, explicação do teste, rótulos nos 4 idiomas, cena na faixa de execução, `KPM_FILE` do cartão 💾, atalho no exercício da aula 05 e o "onde roda" das aulas 05 e 06 reescrito. 8 testes unitários no `test:tudo`, incluindo os casos em que a cadeia deve parar (sem carga, sem fase depois). A versão ao vivo fica para quando o servidor estiver ligado |
| 0.87.0 | 2026-09-11 | **A plataforma de exercícios do professor sai do ar — e os 27 exercícios passam a ser do painel.** O site da CESAR School (React no Cloud Run, que avisava o docente por e-mail) vai ser desligado. Antes disso, o bundle foi **arquivado** em `external/plataforma-cesar/2026-09-11/` (fora do git: material do professor, repo público), com três extratores estáticos: os objetos de configuração de `#data` e `#ric`, o código de cada exercício de `#oran` com as subtelas, e a análise dos quatro interativos com gabaritos. Nenhum exercício foi aberto no site. Eram **27, não 26**: `#data/kpi-dashboard` entrou depois do nosso levantamento. E a **aula 01 do Módulo 05 vale 15, não 9** — o levantamento da 0.73 não viu a quarta tela (arrastar funções para RU/DU/CU). Os 19 que faltavam foram escritos no nosso formato cobrindo os mesmos conceitos do original, nunca copiados: no site a certa era quase sempre a primeira alternativa e os distratores às vezes absurdos. Onde o original errava, a versão nossa corrige (split mais alto não concentra PHY na DU; o SMF, e não o UPF, programa o túnel; a O-RU é gerida pelo M-Plane, não pela O1; SD-RAN também exige Kubernetes; vendor lock-in também existe em vRAN). Três não tinham nota no original e ganharam: **Aventura O-RAN** (22), com o tipo novo **`associar`** (cada item escolhe um alvo, item que aceita mais de um alvo, crédito parcial); **Lab Open5GS** (22), reescrito para o nosso P1; e **Lab de dígitos** (9), que no site **nunca funcionou** (o endpoint de inferência veio vazio) e aqui é `/lab/digitos`: MLP 784→64→10 treinado no MNIST com o mesmo pré-processamento da página (97,6% no teste, pesos de 156 KB, `modelos/treinar_digitos.py` reprodutível byte a byte), com a comparação lado a lado do método antigo, sem centralizar, que erra dígito pequeno no canto. O **Dashboard KPI/NOC** virou `/lab/dashboard-kpi` + exercício de diagnóstico (9). **Revisão adversarial** de todos os exercícios, e ela pegou os **cinco que já estavam no ar** (`data-aula01` a `04` e `data-lab`) com a resposta certa sendo **a alternativa mais longa em 9 de 9 perguntas** e quase sempre a segunda — dava para passar escolhendo a mais comprida. Os meus `data-aula05/06` da 0.86 tinham o mesmo vício. Tudo reequilibrado, e com erros de conteúdo que valiam nota: "3,7 · 8.619 · 82.190 = mediana, média, p95" misturava duas fases (é mediana, média e máximo da recuperação, 172.317; conferido na amostra); a aula 03 ordenava o ETL contra a tabela do slide 47; a aula 02 chamava o lab de ETL quando a aula diz ELT; perguntas que se respondiam umas às outras na mesma página. **O `test:exercicios` passa a impedir que isso volte**: reprova arquivo com a certa como a mais longa em mais de um terço das perguntas ou concentrada numa posição, item do catálogo sem arquivo ou sem pontos, cadeira cruzada com pontos divergentes e qualquer arquivo do painel citando o domínio da plataforma — cada regra provada quebrando de propósito. No painel: o hub da cadeira perdeu o "Abrir ↗", a nota sobre e-mail ao docente e o **formulário de registro manual** (o resultado agora só vem da própria página do exercício); o `POST /api/estudos/resultado` só aceita `{ex, acertos}` e o total vem sempre do catálogo; `plataforma` e `"nosso"` saíram dos quatro catálogos; o glossário não marca mais dentro de `<select>`. De passagem: a bíblia dizia NG-AP na F1-C (é F1AP) |
| 0.86.0 | 2026-09-11 | **A cadeira 4 fica completa: as aulas 05 e 06 entram, a 04 deixa de ser prévia, e os 7 exercícios do `#data` são todos nossos.** Os slides das aulas 05 (*Decisões orientadas por dados*, 84 págs.) e 06 (*Consolidação*, 33 págs.) foram arquivados em `pdfs/03-dados-telecom/` e viraram `e4a05.json` e `e4a06.json`, nos quatro idiomas: painel de BI e cartão de KPI, a matriz sintoma → hipótese → alavanca, **SEHAL** (Situação · Evidência · Hipótese · Ação · Limitação — o acrônimo que o painel esperava desde a 0.71.0 e que eu me recusei a inventar), o closed loop A1 com decision ≠ policy ≠ action ≠ actuation, o before/after do `effect_report`, a rúbrica, o checklist de entrega e a defesa individual. O "onde roda" das duas diz o que **não** temos: o nosso `p2-test-a1` vai do PMS a um **simulador** A1, sem Mediator, consumer nem actuator, então o effect_report da aula não se reproduz aqui. A aula 04 existia desde a 0.64.2 como prévia montada do plano de ensino — o PDF chegou em 26/08 e ninguém a refez; agora foi reescrita na ordem dos slides (14 conceitos, 12 fórmulas) e as traduções acompanharam. Exercícios `#data/aula05` e `#data/aula06` escritos (22 pts cada, `test:exercicios` verde). Relendo a aula 04 contra os slides, o **exercício da aula 04, que conta nota, tinha três respostas que os slides contradizem**: os "quatro elementos" de um KPI (o slide 18 lista sete), a ordem da cadeia (o slide 55 põe QoS/SLA **antes** de QoE) e uma pergunta cuja resposta dependia de a unidade do atraso ser µs — corrigidas, e a de SLI/SLO/SLA, que os slides não cobrem, trocada por KPI × KQI. **A unidade do atraso não está resolvida e as aulas dizem isso:** o log de Indication do slide 66 da aula 01 imprime `DRB.RlcSduDelayDl` em `us`, e o xApp do FlexRIC rotula µs; os slides 43 e 48 da aula 04 e os before/after das aulas 05 e 06 escrevem **ms**. O painel segue em µs e nenhuma aula esconde a divergência. Também: os `index.{en,es,fr}.json` nunca receberam o `"nosso": true` da 0.85 — quem abria o painel em inglês ainda era mandado para a plataforma de fora nos exercícios que já eram nossos; os 7 itens do `#data` agora são nossos nos quatro idiomas. `proximas` saiu do Estudo 4 (a disciplina terminou em 03/09; o README ainda dizia 29/08) |
| 0.85.5 | 2026-08-29 | **O servidor passa a marcar a hora de Brasília.** A AMI vinha em **UTC**, e com a turma em −03 todo log pedia uma conta de cabeça de três horas. Custou caro hoje: eu li "18:28" no journal e não percebi que era **15:28**, o minuto em que a aula do professor começava — reiniciei o painel dois minutos antes da sessão dele, e o log mostra oito logins em treze minutos enquanto o serviço subia. `timedatectl set-timezone America/Sao_Paulo` aplicado no box (o journald guarda em UTC e renderiza no fuso local, então o **histórico inteiro** passou a ser legível sem conversão) e fixado como etapa **0/5 do bootstrap**, idempotente: só age se estiver fora, e aí reinicia o painel — que guarda o fuso de quando subiu e nomeia os arquivos de resultado por ele |
| 0.85.4 | 2026-08-29 | **O deploy passa a se recusar a derrubar quem está usando o painel.** Hoje eu reiniciei o serviço em cima de **três** pessoas conectadas. Tentei medir antes: a expressão de busca dos IPs não casou nada, e eu li o **resultado vazio como resposta** — "movimento baixo". Eram 105, 97 e 86 requisições. O log mostra o processo trocando e um Professor **relogando duas vezes**. Agora `./deploy.sh panel` conta os clientes distintos no journal dos últimos 10 minutos e **aborta**, listando os IPs; `FORCA=1` passa por cima, de propósito — derrubar a sessão de alguém tem de ser decisão, não efeito colateral de comando de rotina. Os dois caminhos testados: com gente, aborta; com `FORCA=1`, avisa e segue. **A lição maior não é sobre deploy:** medição que falha tem de gritar, não devolver zero — foi o mesmo erro que me fez dizer que o `off` "não pegou" quando eu tinha inferido em vez de ler o journal |
| 0.85.3 | 2026-08-29 | **5 de 26 exercícios nossos — e as explicações saíram do Grupo 6.** Entram `#data/aula04` (KPIs e QoE) e `#data/lab` (Projeto prático M09). E os três primeiros foram **neutralizados**: os NÚMEROS ficam — os sete grupos receberam o mesmo dado —, mas some tudo o que era "o projeto do Grupo 6" e "o CP1", porque o painel é de todos. **Onde eu parei e por quê:** `#data/aula05` e `#data/aula06` não têm slide nem aula escrita, só a ementa de uma linha ("CPM, CCO, energy saving, RL no RIC", "temas Wong 8–12"). Escrever exercício que vale nota sobre conteúdo que eu nunca vi seria inventar — falta a fonte |
| 0.85.2 | 2026-08-29 | **Mais dois exercícios nossos: 3 de 26.** `#data/aula02` (Lakes e O-Cloud) e `#data/aula03` (EDA, ETL e BI), 22 pontos e 10 perguntas cada, ancorados nas aulas escritas — e, onde cabe, no **próprio trabalho do Grupo 6**: o Redis do `ric_dbaas` que não serve para treinar, o pico de 172.317 kbps que fez escolher mediana, a correlação de 0,48 que só existe misturando fases, a MAD do `robust-baseline-mad`. O aluno reencontra no exercício o número que viu no relatório |
| 0.85.1 | 2026-08-29 | **A média para passar é 70% — e agora a tela diz isso.** A nota de passagem virou **dado** (`media` no catálogo), num lugar só, em vez de número espalhado. O placar do exercício passou a falar em cima dela: acima de 90 "domina", ≥70 **passou** (caixa e número em verde), até 10 pontos abaixo "faltou pouco" com a distância em pontos percentuais, e mais longe manda voltar para a aula. Na ficha do aluno, **a barra de cada resultado ganhou a linha da média** — sempre na mesma posição, então exercícios de 9 e de 22 pontos se comparam de relance; sem ela o aluno via 64% e não sabia se era bom. O `test:exercicios` reprova catálogo sem `media` |
| 0.85.0 | 2026-08-29 | **Exercícios nossos: a nota passa a ser calculada, não declarada.** Até aqui o aluno fazia o exercício na plataforma da CESAR School e **digitava aqui** quantos pontos tirou. Agora a pergunta acontece dentro do painel. Formato novo em `static/lab/estudos/ex/` (um arquivo por exercício), página `/lab/estudo/{n}/exercicio/{slug}` com dois tipos de bloco — **escolha** e **ordenar** (com crédito parcial por posição certa) —, correção na hora e gravação pelo `/api/estudos/resultado` que já existia; o total continua vindo do **catálogo**, nunca do navegador. Cada pergunta carrega um **`porque`** que aparece depois de responder: é a diferença entre isto e um placar. **Migração sem buraco:** o item do catálogo ganha `"nosso": true` quando o arquivo existe, e só então o link deixa de sair para fora — quem ainda não foi escrito continua funcionando na plataforma antiga. **1 de 26 pronto** (`#data/aula01`, 22 pontos, 10 perguntas). Teste novo `test:exercicios`: reprova soma de pontos divergente do catálogo (o aluno tiraria nota que não existe), pergunta sem `porque`, `ok` fora do intervalo, alternativa repetida, e item marcado "nosso" sem arquivo — os dois primeiros provados quebrando de propósito. O `test:labs` aprendeu que há páginas dirigidas pela ROTA e passou a fingir a URL real |
| 0.84.3 | 2026-08-29 | **O desligamento parou de anunciar sucesso sem olhar.** `switch_project.sh` fazia `|| true` em cada sub-script e depois emitia `ok` **incondicional**; o ramo `off` fechava com `DONE|ok` fixo. E `down_e2_lab.sh` mandava o `down_gnb_oai.sh` para `2>/dev/null || true` — os scripts-folha CONFEREM com pgrep e saem 1, e esse veredito ia para o lixo nos dois níveis, junto com a mensagem do erro. O painel dizia "desligado" sem ter olhado; numa instância **burstable**, isso é o lab seguir queimando crédito com todo mundo achando que parou. Agora o rc vaga até o `DONE|`, sem abortar no meio (o `-e` saiu de propósito: tentar gNB **e** FlexRIC e só depois reportar). Testado em arnês isolado com stubs. **NÃO está no servidor** — a instância foi desligada antes; quando voltar, precisa de `./deploy.sh sync` e `sync-oai`, porque `panel` não leva esses dois arquivos |
| 0.84.2 | 2026-08-29 | **O CI pegou o que eu deixei passar: três traduções defasadas.** O doc da Jornada foi editado duas vezes (0.81.0 e 0.84.0) e as versões en/es/fr ficaram no commit antigo — quem lê noutro idioma não ficava sabendo que as siglas passaram a se explicar na tela. O `docs/i18n/check-parity.py` compara o hash citado no cabeçalho de cada tradução com o último commit do canônico e acusa a defasagem; eu vinha rodando só o passo dos **dicionários** e nunca esse. Seção traduzida para os três idiomas, com as duas tabelas, e marcador `sync:` atualizado |
| 0.84.1 | 2026-08-29 | **Briefing da apresentação + a última lacuna da defesa fechada.** `docs/briefing-apresentacao.md`: o que precisa estar na cabeça antes de entrar e não está no roteiro — estado do sistema medido ao vivo, as duas armadilhas dos três botões ▶, os números recalculados do banco, o que foi corrigido na véspera e por quê, e o aviso de que **`entrega/` é ignorado pelo git**, então os três PDFs não viajam com o `pull`. Da estação nova (core5g-arm64-03): a quarta pergunta do bloco 5 — *qual seria a política A1 se a regra tivesse disparado* — estava respondida **no código** desde sempre e ninguém tinha escrito (`a1_dryrun` no ramo `if fr > 0`: tipo 1, dry-run, `{priorityLevel: 10}`, o mesmo valor do `decision.json` do professor); e os dois geradores de PDF procuravam `~/Área de trabalho`, que no macOS é só o nome exibido — o diretório real é `~/Desktop` —, então a cópia falhava **em silêncio** e a estação nova ficava sem o plano B |
| 0.84.0 | 2026-08-29 | **As siglas do próprio desenho da topologia explicam-se.** O rótulo da linha é a sigla mais nua da tela — "N4" e pronto — e era a única que não tinha para onde apontar. Agora **21 dos 25 rótulos** abrem o balão do glossário (os 4 de fora — HTTPS, Proxy, SQL, Telemetria — não são siglas). Texto de SVG não se envolve num `<span>`, então a marca vai no próprio `<text>`: `Glossario.primeiroTermo()` resolve o rótulo composto ("N11/Nsmf" → N11) e `Glossario.ativar()` liga os eventos sem marcar nada. Sublinhado pontilhado num texto de 10px dentro de uma pílula fica sujo — ali a pista é o cursor e a cor que acende. **8 termos novos**: os serviços SBI que aparecem escritos nas linhas (Namf, Nsmf, Nausf, Nudm, Nudr, Npcf, Nbsf, Nnssf), fechando o diagrama inteiro em 128 termos. E uma **dica na legenda** — uma funcionalidade que ninguém encontra não existe. Teste: `test:topo` confere que os rótulos estão marcados, alcançáveis por teclado, que o balão abre e que a dica está lá; os dois modos provados quebrando de propósito |
| 0.83.0 | 2026-08-29 | **O glossário chega às aulas e aos labs — e cresce para o vocabulário de dados.** De 72 para **120 termos**: entram gerência O-RAN (SMO, O2, PM, FM, VES, RNIB, O-Cloud), a pilha de rádio (RRC, RLC, MAC, PHY, DRB), o que se mede (KPI, KQI, QoE, SLA, UL, DL, os contadores `UEThpUl` e `PrbTotUl`, p95) e o mundo de dados (ETL, ELT, EDA, DIKW, OLTP, OLAP, TSDB, PCA, DNN, MAD, IQR, k-means, Isolation Forest). As 12 páginas do lab passam a marcar. **Regra nova, tirada de olhar a tela:** numa legenda curta marca-se toda ocorrência; num TEXTO de aula, não — a primeira página saiu com **338 sublinhados** e virou um campo pontilhado. Agora `umaVez`: o termo se apresenta uma vez e depois já é vocabulário (67 marcas, não 338). **Nunca dentro de código**: em `<code>`/`<pre>` a sigla é literal, sublinhar ali sugere que o programa mudou. Apelidos de grafia (`Near-RT` → `near-RT`) — eram **91 ocorrências** que o casador, sensível a maiúscula, ignorava. **Achado no caminho:** 28 assets de `/static` nas páginas do lab eram pedidos **sem `?v=`** — a mesma quebra da 0.80.4, agora sem número errado para denunciar, só ausência; `test:paginas` passou a exigir o cache-buster em todo asset. Teste novo `test:labs`: 12 páginas × 2 temas, sem erro, tema aplicado, nenhuma sigla dentro de código, nenhuma sem explicação, nenhuma repetida — os quatro modos provados quebrando de propósito. E `./deploy.sh ssh "comando"` deixou de descartar o comando |
| 0.82.0 | 2026-08-29 | **As cores dos labs de ML fechadas — e o defeito maior não era cor.** Das 12 páginas do lab, **nenhuma guardava o tema**: o botão trocava e esquecia. O professor punha o tema claro para o projetor, clicava numa aula e voltava tudo escuro, em cada uma das 12 telas. Como o botão funcionava, não parecia defeito. Agora todas leem e gravam `c5g-theme`, a mesma chave do painel, e `test:paginas` reprova quem tiver botão sem memória. **Rampa de série de gráfico** (`--serie-1..4`): a categórica **não passa** num scatter e isso foi medido — as 70 combinações de 4 dos 8 degraus reprovam a separação sob protanopia/deuteranopia (ΔE 3,9–7,2; alvo ≥8), porque os oito têm a MESMA lightness, e é a lightness que sobrevive à simulação. A rampa nova dá lightness própria a cada faixa: **CVD ΔE 15,1 claro / 14,3 escuro · visão normal 15,3 / 15,6 · contraste ≥3:1 nos dois temas**, sem exceção condicional; nenhum dos 4 matizes é matiz de estado. Entrou na receita (`paleta.py`) e no `medir.py`. **Console do lab agora é `.superficie-escura`**: é escuro nos dois temas, mas resolvia `--good`/`--warn`/`--bad` pelo tema da PÁGINA — a etiqueta `ok` media **4,46:1 no claro**; agora 8,4:1 nos dois. A legenda captura/retorna media **3,33:1 e 4,03:1**; agora 5,1–9,4:1. **Anel das marcas** passou de `#fff` para `var(--surface)` (branco vira halo no tema escuro). Literais: de **174 para 77**, e os 77 que ficam são do **relatório exportado** — Word não resolve `var()` — agora com comentário dizendo por quê |
| 0.81.0 | 2026-08-28 | **A Jornada do UE explica as próprias siglas.** As legendas eram densas de sigla ("o AMF pede ao SMF a sessão PDU (N11/Nsmf)"): quem já sabia, lia; quem estava aprendendo, travava. Agora o **nome por extenso sai entre parênteses** logo depois do termo e **o mouse (ou o toque, ou o Tab) abre um balão** com *o que é* e *para que serve*. Glossário novo em `static/ops/glossario.js`: **72 termos** em duas camadas separadas de propósito — `TERMOS` guarda o nome oficial 3GPP/O-RAN, que **não se traduz**, e `DICTS` as explicações **nos 4 idiomas**. O módulo marca o texto que **já está escrito**, andando só em nós de texto — nenhuma legenda foi reescrita. Três regras para o texto não inchar: nome por extenso **uma vez por passo**; **nunca** parêntese dentro de parêntese (`N1 (UE↔AMF (Access and…))`) nem dois colados (`E2 (…) (KPM)`); termo sozinho entre parênteses recebe o nome **dentro** deles (`RFSIM — RF simulator`). Título só marcado, sem expandir: é manchete e tem de caber numa linha. **Achado no caminho:** a legenda da caixa do tour usava `--muted` (= `--ink-3`, o degrau de 3:1, que é de apoio e não de texto corrido) e media **2,96:1 no tema claro** — abaixo do próprio piso. Subiu para `--ink-2`: **4,8:1 claro / 8,2:1 escuro**. Testes: `test:i18n:parity` reprova termo sem explicação, explicação sem termo e idioma faltando; `test:topo` percorre as 17 etapas conferindo que toda sigla marcada tem balão com conteúdo e que nenhum parêntese aninhou — os quatro modos de falha foram provados quebrando de propósito |
| 0.80.4 | 2026-08-29 | **"O menu está exatamente igual" — e estava mesmo: o navegador servia a folha velha.** As páginas pediam `/static/...?v=` com o número **escrito à mão**, e havia **nove números diferentes** espalhados: o painel pedia o `tokens.css` da **0.75.0** com o projeto já na 0.80.3. Como `/static` é servido pelo StaticFiles com cache normal, o navegador continuava com a versão antiga e **a tela não mudava depois do deploy** — sem nenhum sinal de que era isso. Agora a página escreve `?v=%VER%` e quem serve troca pelo `VERSION`: **uma fonte só**, que sobe sozinha a cada versão. Novo helper `pagina()` em `core.py` (relê só quando o arquivo muda), usado por todas as rotas que devolvem HTML, e o `test:paginas` passou a **reprovar** número de versão escrito à mão |
| 0.80.3 | 2026-08-29 | **Topologia legível de novo — e o defeito era meu, em dois lugares.** (1) Ao migrar a `topology.html` para o sistema, apaguei o bloco de paleta inteiro e levei junto **`--band-fill` e `--band-stroke`**, que não são cor, são opacidade. Em CSS variável indefinida **falha calada**: `fill-opacity:var(--band-fill)` fica inválida e o SVG usa o inicial, **1** — as bandas viraram blocos sólidos e o nome do grupo, que é da mesma cor, sumiu dentro delas. Restaurado o véu (7% no escuro, 13% no claro). (2) O rótulo saía no degrau de **preencher** da rampa categórica: media **3,0–3,4:1** no tema claro. A rampa ganhou degrau de **texto** (`--cat-N-txt`), como toda família de estado já tinha — cor de sólido e cor de texto são papéis diferentes. Medido depois: **5,9–6,6:1 no claro** e **9,0–9,7:1 no escuro**; rótulos de interface a 13–15:1; linhas a 4,1–6,9:1. Teste novo **`test:vars`** varre o projeto atrás de variável CSS usada e nunca definida — provado removendo `--band-fill` de propósito. A varredura também achou `--console-dim` órfã em 5 páginas de ML, `--surface-inset` na prancha da identidade e três variáveis sem reserva no seletor de idioma |
| 0.80.2 | 2026-08-28 | **Explicação no clique, e não só no `title`.** Os instrumentos do topo já traziam o que cada número significa, mas via `title` nativo do navegador: aparece depois de ~1s, some sozinho, sai no estilo do sistema operacional e não existe no toque — para **apontar um indicador à turma numa projeção**, não serve. Agora um clique abre o mesmo texto num cartão do painel, ancorado no instrumento, com o nome dele no topo, e fica **até ser fechado** (× ou Esc). Vale para os 4 do rádio e para os medidores do servidor. O `title` continua no elemento: é a reserva e é o que o leitor de tela lê |
| 0.80.1 | 2026-08-28 | **O rádio do painel estava quebrado havia 13 versões — e ninguém via.** A faixa acendia, o quadrante acendia, e SNR/MCS/PRB/BLER mostravam **"—"** para sempre. Causa: a **0.67.0** removeu o elemento `rl-sub` e a **linha que escrevia nele ficou**. Ela roda DEPOIS de acender a faixa e ANTES de preencher os números, então a exceção derrubava exatamente a parte que importa — e o `catch {}` **mudo** engolia tudo. Falha silenciosa em laço de telemetria é falha invisível: o painel parecia certo, com quatro traços. Corrigido (linha removida, `catch` passa a registrar no console) e verificado no ar: **SNR 51,0 · MCS 28 · PRB 5/51 · BLER 0,0**. Teste **8** novo: com a API dizendo que o rádio está no ar, a faixa tem de mostrar **número**, não só acender — e os valores têm de vir da API. Provado reintroduzindo o defeito de propósito e vendo falhar |
| 0.80.0 | 2026-08-28 | **A aula sai de dentro do servidor: janela própria.** Servidor é servidor, aula é aula — e a aula abria **dentro do console**, substituindo o resultado do teste e herdando o cabeçalho do painel. Agora abre numa **janela só dela**, com nome fixo (clicar de novo reaproveita a mesma janela). O iframe, a barra de título, o modo `?embed=1` e o CSS que escondia coisas saíram das **12 páginas** do lab. **A função não se perdeu**: o botão de rodar teste continua real e fala com a janela do painel pelo `opener`, trazendo-a para a frente — é lá que o resultado aparece, e a aula não finge que executa. **O cabeçalho encolheu de 507px para 215px** (a cadeira, de 392 para 225): a identidade e os controles dividem UMA linha (a direita estava vazia), o título ocupa a largura inteira, o resumo de oito linhas foi para trás de "sobre esta aula", e a palavra "prévia", que aparecia três vezes, aparece uma. **Aviso de método:** a primeira tentativa de remoção por regex quebrou o JavaScript de **dez páginas** (tirei a linha que ABRIA o `if(EMBED){` e deixei o corpo e a chave). Revertido e refeito casando as chaves, com `node --check` em cada arquivo antes de gravar — os que não compilavam simplesmente não foram escritos |
| 0.79.0 | 2026-08-28 | **Varredura de tudo, no ar: 40 combinações limpas.** Auditoria automática de **20 rotas × 2 temas** no servidor de verdade, logada, procurando erro de JavaScript, acentuação quebrada, recurso 404, tokens ausentes, contraste de corpo e estouro horizontal. Achou e corrigiu: **não havia favicon** (todo acesso gerava 404 e a aba ficava com o ícone genérico) — agora há um, com a assinatura da identidade, a grade de recursos; e o teste `paginas.js` passou a exigi-lo. **A tela de Análise ganhou os 4 idiomas**, inclusive as nove regras de leitura: 81 chaves novas, 987 no total. No caminho, três defeitos: o `T()` testava `window.I18N`, mas `I18N` é `const` global e **const global não vira propriedade de window** — a tela mostrava as chaves cruas (`an.no_ar`); o `apply()` reescrevia o rótulo de estado por cima do valor real a cada ciclo; e o agrupamento de containers punha `a1-sim-*` junto do `ric_*`, o que fazia a leitura acusar como "serviço caído" dois containers **parados havia duas semanas** — corrigido pelas bandas da própria topologia. A regra de carga também mudou: com o rádio no ar, carga alta é **o normal do RFSIM** (gNB + UE consomem ~280% de CPU), e dizer "espere a carga baixar" seria um alarme que nunca apaga. Agora ela explica, e aponta o swap como o sinal que importa |
| 0.78.1 | 2026-08-28 | **Dois defeitos que só o dado real revelou** na tela de Análise. (1) O quadrante de Serviços dizia **"0 no ar de 11"** com nove containers de pé: eu tinha escrito o mapa de estado com os nomes do docker (`running`, `exited`, `healthy`), mas o contrato de `/api/telemetry` é de **três** valores — `up | loading | down`, produzidos por `container_status()`. Tudo caía no `else`. (2) A coluna *quando* mostrava **21/01** em toda linha: `started` é timestamp Unix em **segundos** e `new Date()` interpreta número como milissegundos — era 21 de janeiro de **1970**. Os dois erros têm a mesma raiz: **eu inventei o formato em vez de ler o contrato**, e o meu servidor simulado concordava com a invenção, o que fez a tela passar em todos os testes locais. O simulador foi corrigido para devolver o contrato de verdade. Com o painel no ar e o Projeto 2 subido, a tela agora lê certo: 9 de 11 serviços, SNR 51,0 dB com amostra de 0,1 s, 24 execuções reais — e a leitura apontou sozinha dois serviços fora (`ric_a1mediator`, `ric_dbaas`) e carga de 10 para 4 vCPU |
| 0.78.0 | 2026-08-28 | **Tela de Análise do laboratório** (`/analise`) — o "analisador de serviços" no lugar da parede de badges. Cinco assuntos, cada um num quadrante com moldura e etiqueta encaixada: **servidor** (CPU, RAM, swap, disco e carga, cada um com trilho e limiar), **rádio** (SNR, MCS, BLER e a grade de PRB acesa célula a célula — a assinatura da identidade, literal), **serviços** (um ponto de estado por container), **testes** (a faixa das últimas 24 execuções mais a lista, lida do `/api/results` que já existia) e **leitura e sugestões**. Esta última é o que faz a tela ser análise e não painel: **oito regras** que leem os quatro conjuntos e dizem sempre três coisas — o que eu vi, por que importa, o que fazer. Swap acima de zero, carga acima dos vCPU, disco apertado, serviços fora nomeados, rádio parado há mais de 10 s, SNR abaixo de 20 dB, BLER acima de zero num canal ideal, e falhas repetidas agrupadas por comando. Testada com dois cenários simulados (bancada tranquila e bancada em apuros) nos dois temas, sem uma cor escrita à mão — tudo vem dos papéis |
