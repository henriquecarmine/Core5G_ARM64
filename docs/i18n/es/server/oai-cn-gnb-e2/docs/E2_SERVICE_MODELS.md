<!-- sync: e36a3f4e -->
> 🌐 Traducción al **español** del documento canónico en portugués [`server/oai-cn-gnb-e2/docs/E2_SERVICE_MODELS.md`](../../../../../../server/oai-cn-gnb-e2/docs/E2_SERVICE_MODELS.md). Todos los idiomas: [INDEX](../../../INDEX.md) · sincronización verificada por `docs/i18n/check-parity.py`.

# Explorando Service Models E2 en OAI

Guía práctica para **E2SM-RC**, **E2SM-KPM** y **custom SMs** en el gNB OAI monolítico (RFSIM).

## Mapa de capacidades

| SM | O-RAN / Custom | Qué expone | xApp | ¿Funciona en el lab (222/123)? |
|----|----------------|-------------|------|----------------------------|
| **E2SM-RC** v1.03 | O-RAN | estado RRC, copia de mensajes RRC, UE ID | `xapp_rc_moni` | **Sí** (aperiódico) |
| **E2SM-KPM** v2.03 | O-RAN | PRB, throughput, volumen PDCP… | `xapp_kpm_moni` | OK (slice 222/123 vía `test_e2_kpm.sh`) |
| **MAC** | Custom | KPIs MAC por UE | `xapp_cust_moni` | **Sí** |
| **RLC** | Custom | Estadísticas por bearer | idem | **Sí** |
| **PDCP** | Custom | Estadísticas por bearer | idem | **Sí** |
| **GTP** | Custom | Estadísticas GTP-U NGU | idem | **Sí** (gNB-mono) |
| **SLICE / TC** | Custom | Slice / traffic control | emulador FlexRIC | **No** en el OAI RAN |

## E2SM-RC — el más interesante para el control RAN

Implementación OAI (`openairinterface5g/openair2/E2AP/RAN_FUNCTION/O-RAN/ran_func_rc.c`):

### REPORT Service Style 1 — Message copy (aperiódico)

Eventos disparados cuando el UE completa procedimientos RRC:

| Evento | Cuándo |
|--------|--------|
| **UE ID** | `RRC Setup Complete`, F1 UE Context Setup |
| **RRC Message copy** | `RRC Reconfiguration`, `Measurement Report`, `Security Mode Complete`, `RRC Setup Complete` |

El xApp `xapp_rc_moni` decodifica ASN.1 e imprime UE ID (AMF NGAP ID, RAN UE ID) y contenido RRC.

### REPORT Service Style 4 — UE Information (aperiódico)

| Parámetro | Valores |
|-----------|---------|
| **RRC State Changed To** | `idle`, `inactive`, `connected` |

Disparado en transiciones RRC (ej.: attach → `RRC_CONNECTED`).

### CONTROL Service Style 1 — Radio Bearer Control

| Acción O-RAN | Comportamiento OAI |
|------------|-------------------|
| QoS flow mapping | PoC: **creación de un nuevo DRB** (OAI no multiplexa múltiples QoS flows en un DRB) |

Probar con `xapp_kpm_rc` (monitorea KPM + envía RC Control). El control real en el RAN es limitado — ver branch `qoe-e2` upstream para la demo completa.

## Scripts de exploración

```bash
cd ric/code/oai-cn-gnb-e2

# Lab no ar
./scripts/up_e2_lab.sh

# Exploração rápida (custom + RC)
./scripts/explore_e2_sm.sh quick

# Aprofundar RC + PoC KPM/RC control
./scripts/explore_e2_sm.sh rc

# O-RAN KPM + RC agregado
./scripts/explore_e2_sm.sh oran

# Camadas L2/L3 detalhadas
./scripts/explore_e2_sm.sh layers

# Tudo
./scripts/explore_e2_sm.sh full
```

Pruebas individuales:

```bash
XAPP_DURATION=30 ./scripts/test_e2_sm.sh rc       # só RC
XAPP_DURATION=30 ./scripts/test_e2_sm.sh kpm_rc  # KPM + RC control PoC
XAPP_DURATION=30 ./scripts/test_e2_sm.sh gtp     # MAC/RLC/PDCP/GTP + DB sqlite
```

### Generar eventos RC durante la prueba

RC es **aperiódico** — las INDICATIONs aparecen en attach/detach o handover. Para ver eventos durante `xapp_rc_moni`:

```bash
# Terminal 1: xApp RC (30s)
XAPP_DURATION=30 ./scripts/test_e2_sm.sh rc

# Terminal 2 (nos primeiros 10s): reiniciar UE para forçar RRC Setup
./scripts/down_gnb_oai.sh   # para gNB+UE
./scripts/up_gnb_oai.sh     # sobe de novo → novo RRC attach
```

## Qué buscar en los logs

```bash
# RC: state change, UE ID, mensagens RRC
grep -iE 'RRC connected|UE ID|RRCReconfiguration|INDICATION' logs/xapp_rc_moni.log

# Custom: métricas periódicas por UE
grep -iE 'MAC|RLC|PDCP|throughput|INDICATION' logs/xapp_cust_moni.log

# Agente E2 no gNB
grep -iE '\[E2 AGENT\].*RC|signal_rrc|signal_ue_id' logs/gnb_oai.log
```

Éxito RC típico:

```
UE ID type = gNB, amf_ue_ngap_id = ...
RAN Parameter Value = RRC connected
RRC Message ... RRCSetupComplete
```

## E2SM-KPM — métricas O-RAN

Métricas 3GPP TS 28.552: `DRB.PdcpSduVolumeDL/UL`, `DRB.UEThpDl/Ul`, `RRU.PrbTotDl/Ul`, etc.

Los xApps `xapp_kpm_moni` / `xapp_kpm_rc` (FlexRIC `dev`) usan por defecto **SST=222, SD=123** (`KPM_SST` / `KPM_SD`).

```bash
./scripts/test_e2_kpm.sh
./scripts/explore_e2_sm.sh kpm
KPM_SST=222 KPM_SD=123 XAPP_DURATION=45 ./scripts/test_e2_kpm.sh
```

`KPM_TRAFFIC=1` (por defecto) genera ping al DN durante la prueba para métricas de volumen/throughput.

PoC closed-loop: `KPM_SST=222 KPM_SD=123 ./scripts/test_e2_sm.sh kpm_rc`

## Custom SMs — datos offline (ML/AI)

`xapp_gtp_mac_rlc_pdcp_moni` graba en `/tmp/xapp_db_*` (SQLite):

```bash
ls /tmp/xapp_db_*
# sqlitebrowser /tmp/xapp_db_<timestamp>
```

Structs de referencia (código OAI):

- MAC: `mac_ue_stats_impl_t`
- RLC: `rlc_radio_bearer_stats_t`
- PDCP: `pdcp_radio_bearer_stats_t`
- GTP: `gtp_ngu_t_stats_t`

## xApps compilados localmente

Los xApps dedicados (RC, KPM, KPM+RC) viven en el submodule FlexRIC:

```
openairinterface5g/openair2/E2AP/flexric/build/examples/xApp/c/monitor/
  xapp_rc_moni
  xapp_kpm_moni
  xapp_gtp_mac_rlc_pdcp_moni
openairinterface5g/openair2/E2AP/flexric/build/examples/xApp/c/kpm_rc/
  xapp_kpm_rc
```

Compilar (automático en `explore_e2_sm.sh`):

```bash
cd openairinterface5g/openair2/E2AP/flexric/build
cmake .. -GNinja -DE2AP_VERSION=E2AP_V2 -DKPM_VERSION=KPM_V2_03
ninja xapp_rc_moni xapp_kpm_moni xapp_kpm_rc xapp_gtp_mac_rlc_pdcp_moni
```

## Próximos pasos interesantes

| Objetivo | Camino |
|----------|---------|
| RC Control real (nuevo DRB) | Branch upstream `qoe-e2` o `xapp_kpm_rc` + tráfico |
| KPM con slice 222/123 | `./scripts/test_e2_kpm.sh` + `flexric-lib/` (submodule dev) |
| CU/DU split + E2 | gNB split + E2 agent en CU-CP/CU-UP/DU |
| O-RAN SC nearRT-RIC | Puerto E2AP 36422, framework xDevSM |
| Wireshark E2AP | Capturar SCTP :36421 localhost |

## Referencias

- [E2_FLEXRIC.md](E2_FLEXRIC.md) — operación del lab
- [OAI E2AP README](../../../../../../server/oai-cn-gnb-e2/openairinterface5g/openair2/E2AP/README.md)
- O-RAN E2SM-RC v01.03, E2SM-KPM v2.03
