 ...
 *
 * xApp customizado — UE-TP-rApp (extensão obrigatória do Projeto 2).
 *
 * Cópia de xapp_kpm_moni.c com a lógica de assinatura E2SM-KPM intacta
 * (mesma negociação de Report Style 1/4, mesmo filtro de slice via
 * KPM_SST/KPM_SD) — só o que é exibido no callback de indicação muda:
 * em vez de despejar toda medição recebida, destaca DRB.UEThpDl/UEThpUl
 * (throughput por UE, 3GPP TS 28.552) e mantém uma média móvel
 * exponencial (EWMA) por direção como "previsão" do próximo valor —
 * o núcleo do conceito de UE-TP-rApp (estimar o throughput esperado de
 * um UE a partir do histórico de KPM).
 */
...
// EWMA por direção: nova_previsao = alpha*amostra + (1-alpha)*previsao.
// alpha alto (0.3) reage rápido a variações — adequado pro período curto
// (1s) deste lab; um rApp real ajustaria alpha por UE/cenário.
static double const EWMA_ALPHA = 0.3;
static double ewma_dl = -1.0;
static double ewma_ul = -1.0;

static
void log_ue_thp(const char *name_str, double value, const char *unit)
{
  if (strcmp(name_str, "DRB.UEThpDl") == 0) {
    ewma_dl = (ewma_dl < 0) ? value : EWMA_ALPHA * value + (1 - EWMA_ALPHA) * ewma_dl;
    printf(">>> [UE-TP-rApp] DRB.UEThpDl = %8.1f %s   | previsão (EWMA) próxima amostra = %8.1f %s\n", value, unit, ewma_dl, unit);
  } else if (strcmp(name_str, "DRB.UEThpUl") == 0) {
    ewma_ul = (ewma_ul < 0) ? value : EWMA_ALPHA * value + (1 - EWMA_ALPHA) * ewma_ul;
    printf(">>> [UE-TP-rApp] DRB.UEThpUl = %8.1f %s   | previsão (EWMA) próxima amostra = %8.1f %s\n", value, unit, ewma_ul, unit);
  }
}
...
void log_int_value(const char *name_str, const label_info_lst_t label_info, const meas_record_lst_t meas_record)
{
  ...
  if (label_info.noLabel != NULL) {
    ...
    log_ue_thp(name_str, (double)meas_record.int_val, name_unit ? name_unit : "");
...
void log_real_value(const char *name_str, const label_info_lst_t label_info, const meas_record_lst_t meas_record)
{
  ...
  log_ue_thp(name_str, meas_record.real_val, name_unit ? name_unit : "");
}
...
static int lab_kpm_sst(void)
{
  const char *e = getenv("KPM_SST");
  return e ? atoi(e) : 222;
}

static int lab_kpm_sd(void)
{
  const char *e = getenv("KPM_SD");
  if (!e)
    return 123;
  if (strcmp(e, "any") == 0 || strcmp(e, "-1") == 0)
    return -1;
  return (int)strtol(e, NULL, 0);
}

static
test_info_lst_t filter_predicate(test_cond_type_e type, test_cond_e cond, int sst, int sd)
{
...
kpm_act_def_t fill_report_style_4(ric_report_style_item_t const* report_item)
{
  ...
  act_def.frm_4.matching_cond_lst[0].test_info_lst = filter_predicate(type, condition, lab_kpm_sst(), lab_kpm_sd());
...
int main(int argc, char* argv[])
{
  ...
  printf("[UE-TP-rApp] Connected E2 nodes = %d\n", nodes.len);
