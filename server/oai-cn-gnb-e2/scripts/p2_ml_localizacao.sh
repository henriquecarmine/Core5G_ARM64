#!/usr/bin/env bash
# p2_ml_localizacao.sh — roda o experimento Localization (scikit-learn) e
# streama a tabela de métricas ao vivo no console do painel.
#   Caso do artigo Ngo et al. 2024 (Tabela 3), recorte Instance, dados reais do
...
EXP="$SCRIPT_DIR/ml/localization_experiment.py"
# python do venv do painel (numpy + scikit-learn vendorizados); fallback p/ PATH
PY="${PANEL_PY:-$HOME/server/panel/.venv/bin/python3}"
[ -x "$PY" ] || PY="$(command -v python3 || true)"

section "Localization-rApp — classificação de andar (recorte Instance)"
info "Prevê em qual andar (4/5/6) o UE está usando só as KPMs de rádio do RIC."
kv "Artigo" "Ngo et al. 2024, Tabela 3 (dados do walk test SUTD)"
...
if "$PY" -u "$EXP" --data "$DATA_DIR"; then
    summary "treinou os 5 classificadores da Tabela 3 e imprimiu as métricas" \
            "Gradient Boosting (~XGBoost) reproduz o artigo (~84%)" ok
else
    rc=$?
    err "o experimento falhou (rc=$rc) — o venv tem scikit-learn instalado?"
    summary "rodou o experimento Localization" "falhou ao treinar" err
    exit "$rc"
fi
