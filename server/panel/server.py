...
def lab_hub_page() -> FileResponse:
    return FileResponse(STATIC_DIR / "lab-hub.html", headers=NO_CACHE)


@app.get("/lab/fundamentos")
def lab_fundamentos_page() -> FileResponse:
    return FileResponse(STATIC_DIR / "lab-fundamentos.html", headers=NO_CACHE)


@app.get("/lab/regressao")
def lab_regressao_page() -> FileResponse:
    return FileResponse(STATIC_DIR / "lab-regressao.html", headers=NO_CACHE)


@app.get("/lab/classificacao")
...
@app.get("/lab/anomalia")
def lab_anomalia_page() -> FileResponse:
    return FileResponse(STATIC_DIR / "lab-anomalia.html", headers=NO_CACHE)
...
def lab_question(payload: dict, request: Request) -> JSONResponse:
    user, _, email, name = _parse_session(request)
    if not user:
