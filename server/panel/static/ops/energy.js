...
(function () {
  ...
  var CSS = [
    ...
    '  font-size:16px;line-height:1;width:auto!important;min-width:0!important;color:var(--ink-3)}',
    '.en-btn.lit{opacity:1;cursor:pointer;color:var(--warn)!important;text-shadow:0 0 8px rgba(245,159,0,.5)}',
    ...
    '  width:min(680px,94vw);max-height:88vh;overflow-y:auto;background:var(--bg);',
    '  border:1px solid var(--line-2);border-radius:12px;padding:18px 20px;color:var(--console-ink);',
    '  font-family:-apple-system,"Segoe UI",sans-serif;font-size:12.5px;line-height:1.5;',
    '  box-shadow:0 16px 48px rgba(0,0,0,.6)}',
    '.en-mod h3{margin:0 0 2px;font-size:15px;color:var(--ink)}',
    '.en-mod .sub{color:var(--ink-3);font-size:11px;margin-bottom:12px}',
    '.en-mod .x{position:absolute;top:10px;right:14px;cursor:pointer;color:var(--ink-3);font-size:15px}',
    '.en-t{width:100%;border-collapse:collapse;margin:6px 0 4px}',
    '.en-t th{font-size:9.5px;letter-spacing:.07em;text-transform:uppercase;color:var(--ink-3);',
    '  text-align:left;padding:5px 8px;border-bottom:1px solid var(--line)}',
    '.en-t td{padding:6px 8px;border-bottom:1px solid var(--surface);vertical-align:top}',
    '.en-t td.n1{color:var(--good-text);font-weight:600}',
    '.en-t td.x86{color:var(--ink-3)}',
    '.en-t td.dl{color:var(--warn);font-weight:700;white-space:nowrap}',
    '.en-t .live{color:var(--accent-text);font-size:10px;margin-left:4px}',
    '.en-t .est{color:var(--ink-3);font-size:10px;margin-left:4px}',
    '.en-t .src{color:var(--ink-3);font-size:10px}',
    '.en-bar{height:9px;border-radius:5px;background:var(--surface);overflow:hidden;margin-top:3px}',
    '.en-bar>i{display:block;height:100%;border-radius:5px}',
    '.en-verd{margin:10px 0 4px;padding:10px 12px;border:1px solid var(--g-8);border-radius:9px;background:color-mix(in srgb, var(--good) 13%, transparent)}',
    '.en-verd b{color:var(--good-text)}',
    '.en-why{margin-top:10px;border-top:1px solid var(--line);padding-top:9px}',
    '.en-why>span{cursor:pointer;color:var(--accent-text)}',
    '.en-why .body{display:none;margin-top:7px;color:var(--console-ink);font-style:italic}',
    '.en-why.open .body{display:block}',
    '.en-cav{margin-top:9px;color:var(--ink-3);font-size:10.5px}',
    '.en-src{margin-top:8px;font-size:10.5px;color:var(--ink-3)}',
    '.en-src a{color:var(--accent-text);text-decoration:none}',
  ].join('\n');
  ...
  function render(d, tele) {
    ...
    var liveW = cpu !== null && d.power
      ? '⚡ AGORA (CPU ' + cpu + '%): N1 ≈ <b style="color:var(--good-text)">' + watts(d.power.n1, cpu).toFixed(1) + ' W</b> · x86 equivalente ≈ <b style="color:var(--ink-3)">' + watts(d.power.x86, cpu).toFixed(1) + ' W</b> <span class="est">interp. linear Teads ' + d.power.src + '</span>'
      : '';
    var runLine = last
    ...
    var verd = '<div class="en-verd"><b>e. VEREDITO — média ponderada:</b> N1 <b>' + v.n1 + '</b> × ' + v.x86 + ' x86_64'
      + '<div class="en-bar"><i style="width:' + v.n1 + '%;background:var(--good)"></i></div>'
      + '<div class="en-bar"><i style="width:' + v.x86 + '%;background:var(--ink-3)"></i></div>'
      + '<div style="margin-top:6px;font-size:11px;color:var(--ink-3)">' + v.pesos + '</div>'
    ...
    var srcs = d.sources.map(function (s) {
      return '[' + s.n + '] <a href="' + s.url + '" target="_blank" rel="noopener">' + s.title + '</a> <span style="color:var(--ink-3)">(' + s.side + ')</span>';
    }).join(' · ');
  ...
  function open() {
    Promise.all([fetchData(), fetch('/api/telemetry').then(function (r) { return r.json(); }).catch(function () { return null; })])
      .then(function (res) {
        ...
        var m = document.createElement('div'); m.className = 'en-mod superficie-escura';
    ...
    begin: function (cmd) { last = { cmd: cmd, t0: Date.now(), ok: null, dur: 0 }; },
      ...
      if (out && last && last.dur > 1 && isTest) {
        fetchData().then(function (d) {
          ...
          div.style.cssText = 'margin:4px 0;padding:3px 10px;border-left:2px solid var(--warn);font-size:10.5px;color:var(--ink-3)';
          div.innerHTML = '⚡ ' + Math.round(t) + 's · N1 ≈ <b style="color:var(--good-text)">' + eN1 + ' Wh</b> · x86 eq. ≈ ' + eX + ' Wh · <b style="color:var(--good-text)">−52%</b> '
            + '<span style="color:var(--ink-3)">(estimado; teto@100%)</span> · '
            + '<a href="#" style="color:var(--accent-text)" onclick="Energy._open();return false">tabela ⚡</a>';
          out.appendChild(div);
