...
(function () {
  ...
  var SCENES = [
    ...
    { name: "closedloop", match: /^p2-closed-loop$/,
      nodes: [{ id: 'kpm', txt: '💾 KPM' }, { id: 'mad', txt: '🧮 MAD' },
              { id: 'dec', txt: '🧭 decision' }, { id: 'pol', txt: '📜 policy' },
              { id: 'act', txt: '⚙️ atuação' }, { id: 'rep', txt: '📊 effect_report' }],
      edges: ['', '', '', 'A1', ''],
      triggers: [
        { re: /2\. Baseline|treino do MAD/i, to: 'mad' },
        { re: /4\. Decisao|decision\.json/i, to: 'dec' },
        { re: /5\. A1|Policy A1 candidata/i, to: 'pol' },
        { re: /6\. Atuacao|action_request \(/i, to: 'act' },
        { re: /7\. Depois|8\. Rollback|Veredito/i, to: 'rep' },
      ] },
    // ---- SMO do O-RAN SC (Gestão, Orquestração e Automação): um teste por item da avaliação ----
    { name: "smoarq", match: /^smo-arquitetura$/,
      nodes: [{ id: 'cont', txt: '🐳 3 camadas' }, { id: 'gw', txt: '🚪 gateway' },
              { id: 'ctl', txt: '🧭 controlador O1' }, { id: 'ne', txt: '📡 O-DU · O-RU' }, { id: 'saude', txt: '❤️ saúde' }],
      edges: ['', 'HTTPS', 'NETCONF', ''],
      triggers: [
        { re: /2\. Serviços|serviços roteados/i, to: 'gw' },
        { re: /3\. Componentes O-RAN/i, to: 'ctl' },
        { re: /O1 \(SMO ↔ O-DU\)/i, to: 'ne' },
        { re: /4\. Saúde/i, to: 'saude' },
      ] },
    { name: "smoo1", match: /^smo-o1-leitura$/,
      nodes: [{ id: 'painel', txt: '🖥 painel' }, { id: 'gw', txt: '🚪 gateway' },
              { id: 'ctl', txt: '🧭 controlador' }, { id: 'odu', txt: '📡 O-DU' }, { id: 'cfg', txt: '📄 gNB-DU · célula' }],
      edges: ['HTTPS', 'RESTCONF', 'NETCONF', 'YANG'],
      triggers: [
        { re: /1\. O elemento está montado/i, to: 'gw' },
        { re: /Conexão\s+connected/i, to: 'ctl' },
        { re: /2\. Leitura pela O1/i, to: 'odu' },
        { re: /leitura pela O1 em/i, to: 'cfg' },
      ] },
    { name: "smoprov", match: /^smo-o1-prov$/,
      nodes: [{ id: 'le', txt: '🔍 estado atual' }, { id: 'patch', txt: '📝 PATCH' },
              { id: 'ctl', txt: '🧭 controlador' }, { id: 'odu', txt: '📡 datastore da O-DU' }, { id: 'volta', txt: '↩️ desfazer' }],
      edges: ['', 'RESTCONF', 'edit-config', ''],
      triggers: [
        { re: /2\. Escrita/i, to: 'patch' },
        { re: /aceita pelo controlador/i, to: 'ctl' },
        { re: /3\. A mudança chegou/i, to: 'odu' },
        { re: /4\. Desfazer/i, to: 'volta' },
      ] },
    { name: "smofm", match: /^smo-falhas$/,
      nodes: [{ id: 'odu', txt: '📡 O-DU (alarme)' }, { id: 'ves', txt: '📨 coletor VES' },
              { id: 'kafka', txt: '📬 Kafka · falhas' }, { id: 'limpa', txt: '🧹 limpeza' }, { id: 'guarda', txt: '🗄 o que o SMO guarda' }],
      edges: ['VES', '', '', ''],
      triggers: [
        { re: /VES → coletor\s+HTTP 202/i, to: 'ves' },
        { re: /no tópico de falhas/i, to: 'kafka' },
        { re: /2\. O elemento limpa/i, to: 'limpa' },
        { re: /3\. O que o SMO guarda/i, to: 'guarda' },
      ] },
    { name: "smopm", match: /^smo-telemetria$/,
      nodes: [{ id: 'cfg', txt: '⚙️ PM da O-DU' }, { id: 'arq', txt: '📄 arquivo 28.532' },
              { id: 'ves', txt: '📨 FileReady' }, { id: 'kafka', txt: '📬 Kafka' }, { id: 'vida', txt: '❤️ heartbeats' }],
      edges: ['', 'VES', '', ''],
      triggers: [
        { re: /2\. O último arquivo/i, to: 'arq' },
        { re: /3\. O aviso que chega/i, to: 'ves' },
        { re: /avisado do arquivo pronto/i, to: 'kafka' },
        { re: /4\. Quem está vivo/i, to: 'vida' },
      ] },
    { name: "smolcm", match: /^smo-ciclo-vida$/,
      nodes: [{ id: 'op', txt: '▶️ em operação' }, { id: 'stop', txt: '⏹ encerrar' }, { id: 'perda', txt: '📉 SMO percebe' },
              { id: 'start', txt: '🔁 instanciar' }, { id: 'ch', txt: '📞 call home' }, { id: 'log', txt: '🗄 connectionlog' }],
      edges: ['', '', '', 'NETCONF', ''],
      triggers: [
        { re: /2\. Encerrar/i, to: 'stop' },
        { re: /o SMO percebeu/i, to: 'perda' },
        { re: /3\. Instanciar/i, to: 'start' },
        { re: /call home → montado/i, to: 'ch' },
        { re: /4\. O que ficou registrado/i, to: 'log' },
      ] },
    { name: "smoocloud", match: /^smo-ocloud$/,
      nodes: [{ id: 'pool', txt: '🏗 recursos' }, { id: 'dms', txt: '🐳 implantação' },
              { id: 'nfs', txt: '📦 funções' }, { id: 'o2', txt: '⚠️ O2' }],
      edges: ['', '', ''],
      triggers: [
        { re: /2\. Gerenciador de implantação/i, to: 'dms' },
        { re: /3\. Funções implantadas/i, to: 'nfs' },
        { re: /4\. O que ficaria com o O2/i, to: 'o2' },
      ] },
  ...
  var INFOS = {
    ...
    smoarq: {
      cont: { q: 'As 3 camadas do SMO do O-RAN SC, lidas do Docker: plataforma (common), gerência (oam) e elementos gerenciados (network).', o: 'servidor: <b>server/smo/</b> · docker compose do repositório o-ran-sc/oam', d: 'Todas as imagens em arm64: 7 foram construídas aqui (troca de base ou compilação), o resto é oficial multi-arch.' },
      gw: { q: 'A porta única do SMO: o Traefik termina o HTTPS e roteia pelo nome de cada serviço.', o: 'rotas lidas da API do próprio gateway', d: 'Identidade, console, O1, eventos VES, barramento e topologia; o NETCONF call home entra pelas portas 4334/4335. Só em localhost neste servidor.' },
      ctl: { q: 'O controlador SDN-R (OpenDaylight): o cliente NETCONF do SMO, que monta cada elemento pelos modelos YANG.', o: 'RESTCONF: <b>/rests/data/network-topology…/topology-netconf</b>', d: 'É ele que implementa a O1 do lado do SMO.' },
      ne: { q: 'Os elementos O-RAN sob gerência agora e quantos modelos YANG cada um anunciou.', o: 'O-DU (O1) e O-RU híbrido (M-plane), simulados pelo pynts', d: 'A1 e O2 não fazem parte desta solução: o teste diz isso na tela.' },
      saude: { q: 'Os três serviços que um operador usaria, testados pelo gateway.', o: 'Keycloak (realm onap), controlador (/ready) e ODLUX', d: 'HTTP 200 nos três = SMO pronto para uso.' },
    },
    smoo1: {
      painel: { q: 'O teste roda no servidor, como um operador que chama o SMO.', o: '<b>server/smo/smo_o1_leitura.sh</b>', d: 'Nada é lido direto do simulador: tudo passa pela O1.' },
      gw: { q: 'A entrada HTTPS do SMO, pelo nome controller.dcn.', o: 'Traefik em 127.0.0.1:8443', d: 'O gateway autentica o TLS e entrega ao controlador.' },
      ctl: { q: 'O controlador traduz RESTCONF (HTTP/JSON) em NETCONF (SSH/TLS e XML) para o elemento.', o: 'OpenDaylight · RFC 8040', d: 'É por isso que uma tela web consegue ler um equipamento que só fala NETCONF.' },
      odu: { q: 'A O-DU simulada, montada no controlador pela conexão NETCONF que ela mesma abriu (call home).', o: 'pynts-o-du-o1 · netopeer2 + sysrepo', d: '154 modelos YANG anunciados: O-RAN, 3GPP e IETF.' },
      cfg: { q: 'O que foi lido: a árvore 3GPP ManagedElement → GNBDUFunction → NRCellDU.', o: 'modelos _3gpp-nr-nrm-gnbdufunction e _3gpp-nr-nrm-nrcelldu', d: 'gNBId, nome da gNB-DU, PCI, ARFCN, SSB e as fatias por PLMN — o modelo de rede que a gerência de uma operadora usa.' },
    },
    smoprov: {
      le: { q: 'O valor antes da mudança, lido pela O1.', o: 'GNBDUFunction / attributes / gNBDUName', d: 'Guardado para o desfazer — e para o caso de algo falhar no meio.' },
      patch: { q: 'A escrita: um RESTCONF PATCH com só o atributo que muda.', o: 'corpo JSON impresso na tela', d: 'Parâmetro escolhido por ser visível e inofensivo: o nome da gNB-DU.' },
      ctl: { q: 'O controlador aceita e vira NETCONF edit-config no elemento.', o: 'resposta HTTP 200 com o tempo', d: 'A escrita é transacional: ou entra inteira ou não entra.' },
      odu: { q: 'A prova: o valor novo lido de novo pela O1 E no datastore running dentro da O-DU.', o: '<b>sysrepocfg</b> no contêiner do elemento', d: 'Mostra que a mudança chegou ao equipamento, não só à tela do SMO.' },
      volta: { q: 'O desfazer: a mesma escrita com o valor original, conferida no elemento.', o: 'o elemento termina como começou', d: 'É o que um provisionamento de verdade precisa: aplicar, verificar e poder voltar.' },
    },
    smofm: {
      odu: { q: 'O alarme linkDown (enlace do fronthaul caiu), em nome da O-DU.', o: 'formato VES 7.2.1, domínio fault', d: 'A O-DU simulada não traz receita de alarmes; o teste faz o papel dela com o formato que ela usaria.' },
      ves: { q: 'O coletor VES recebe o evento por HTTPS e valida contra o esquema.', o: 'ves-collector.dcn · /eventListener/v7', d: 'HTTP 202 = aceito para entrega.' },
      kafka: { q: 'O evento publicado no tópico de falhas do barramento.', o: 'Kafka: <b>unauthenticated.SEC_FAULT_OUTPUT</b> (lido pelo kafka-bridge)', d: 'Qualquer consumidor assina daqui: análise, rApps, o kafka-ui.' },
      limpa: { q: 'O mesmo alarme encerrado, com severidade NORMAL.', o: 'mesmo tópico', d: 'Um alarme sem limpeza vira ruído permanente na operação.' },
      guarda: { q: 'Os dois canais de falha: VES no barramento e notificações NETCONF no banco do controlador.', o: 'MariaDB do controlador: faultcurrent e faultlog', d: 'Cada fonte de falha chega por um caminho diferente — e o SMO junta as duas.' },
    },
    smopm: {
      cfg: { q: 'O que a O-DU mede e de quanto em quanto tempo.', o: '/data/performance-management/index.json no elemento', d: '3 contadores DRB a cada 60 s, válidos por 180 s.' },
      arq: { q: 'O arquivo de medidas mais novo, lido no próprio elemento.', o: '/ftp da O-DU · formato 3GPP TS 28.532 (XML)', d: 'É o formato que um equipamento real entrega à gerência de desempenho.' },
      ves: { q: 'O aviso VES FileReady: "há um arquivo novo, está neste endereço".', o: 'domínio stndDefined, tópico SEC_3GPP_PERFORMANCEASSURANCE_OUTPUT', d: 'O SMO não precisa varrer os elementos: eles avisam.' },
      kafka: { q: 'O último aviso lido no barramento, com o endereço do arquivo.', o: 'kafka-bridge do SMO', d: 'Daqui um coletor de PM buscaria o arquivo por SFTP.' },
      vida: { q: 'Quantos eventos o SMO já recebeu por tipo: medidas, heartbeats e registros de PNF.', o: 'offsets dos tópicos no Kafka', d: 'Heartbeat é o "estou vivo" periódico; registro é o "cheguei" ao ligar.' },
    },
    smolcm: {
      op: { q: 'A função de rede em operação: o O-RU híbrido conectado ao controlador.', o: 'estado NETCONF no controlador', d: 'É o ponto de partida do ciclo.' },
      stop: { q: 'Encerrar a função de rede.', o: '<b>docker stop</b>', d: 'Num O-RAN completo, quem encerra é o O-Cloud a pedido do SMO, pelo O2 (DMS).' },
      perda: { q: 'Quanto tempo o controlador leva para perceber que o elemento sumiu.', o: 'estado NETCONF consultado a cada 2 s', d: 'Sem isso a gerência ficaria mandando configuração para quem não existe.' },
      start: { q: 'Instanciar a função de rede de novo.', o: '<b>docker start</b>', d: 'O teste nunca deixa o elemento parado, nem se algo falhar.' },
      ch: { q: 'O elemento liga para o SMO (NETCONF call home) e é montado de novo.', o: 'portas 4334/4335 do gateway → controlador', d: 'Ninguém configura o elemento no controlador: ele se apresenta sozinho.' },
      log: { q: 'O que ficou registrado no banco do controlador, com horário.', o: 'MariaDB: tabela connectionlog', d: 'Unmounted → Mounted → Connected: a história do ciclo, auditável.' },
    },
    smoocloud: {
      pool: { q: 'O pool de recursos do O-Cloud do lab: arquitetura, vCPU, memória, disco e carga.', o: 'lido do próprio servidor', d: 'É o que um O2 IMS inventariaria e publicaria ao SMO.' },
      dms: { q: 'Quem implanta as funções de rede aqui: o Docker e as implantações do SMO.', o: 'docker compose: smo-common, smo-oam, smo-network', d: 'É o papel que o O2 DMS teria.' },
      nfs: { q: 'Cada função implantada e quanto de CPU e memória consome agora.', o: 'docker stats', d: 'O total mostra o custo do SMO neste O-Cloud.' },
      o2: { q: 'A lacuna: o OAM do O-RAN SC não implementa O2.', o: 'O2 no O-RAN SC = projeto INF (IMS/DMS sobre StarlingX)', d: 'É a resposta honesta aos itens O2 e O-Cloud da avaliação.' },
    },
    tema: {
