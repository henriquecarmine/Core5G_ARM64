...
(function () {
  ...
  var TERMOS = {
    ...
    'RESTCONF': 'RESTful NETCONF · RFC 8040',
    'M-plane': 'Management plane · Open Fronthaul',
    'PNF': 'Physical Network Function',
    'TAPI': 'Transport API · ONF',
  ...
  pt: {
    ...
    'NETCONF.o': 'O protocolo de configuração de rede do IETF: lê e grava a configuração de um equipamento em transações.',
    'NETCONF.p': 'É o que a O1 e o M-plane usam: o SMO muda a configuração sem entrar no equipamento.',
    'YANG.o': 'A linguagem que descreve o que um equipamento sabe fazer: cada parâmetro, tipo e limite.',
    'YANG.p': 'O equipamento anuncia seus modelos YANG e o SMO passa a saber o que pode configurar nele.',
    'RESTCONF.o': 'O mesmo modelo YANG do NETCONF, acessado por HTTP e JSON.',
    'RESTCONF.p': 'Deixa telas web e scripts falarem com o controlador sem cliente NETCONF.',
    'M-plane.o': 'O plano de gerência do Open Fronthaul, entre a O-DU ou o SMO e o O-RU.',
    'M-plane.p': 'Configura e supervisiona o rádio: direto do SMO, no modelo híbrido, ou através da O-DU, no hierárquico.',
    'PNF.o': 'Uma função de rede em hardware próprio, como um O-RU — o oposto de uma função virtualizada.',
    'PNF.p': 'Ao ligar, ela se anuncia ao SMO com um pnfRegistration para entrar no inventário.',
    'TAPI.o': 'A API padrão de topologia e conectividade de transporte, da ONF.',
    'TAPI.p': 'Descreve quem se liga a quem na rede num formato que qualquer SMO entende.',
    'OAuth.o': 'O padrão de autorização por token: quem faz login recebe uma credencial temporária.',
    'OAuth.p': 'O serviço confere o token em vez de guardar senhas — no SMO, o Keycloak emite e o controlador valida.',
    'Kafka.o': 'Um barramento de mensagens: produtores publicam em tópicos, consumidores assinam.',
    'Kafka.p': 'Desacopla quem gera eventos de quem os usa — o VES publica, a análise lê quando quiser.',
    'Keycloak.o': 'Servidor de identidade de código aberto: usuários, papéis e login único.',
    'Keycloak.p': 'Centraliza quem pode usar o SMO e com qual papel.',
    'OpenDaylight.o': 'Controlador de rede de código aberto da Linux Foundation, escrito em Java.',
    'OpenDaylight.p': 'É a base do controlador SDN-R do O-RAN SC, que fala NETCONF com os elementos.',
  ...
  en: {
    ...
    'NETCONF.o': 'The IETF network configuration protocol: reads and writes a device’s configuration in transactions.',
    'NETCONF.p': 'It is what O1 and the M-plane use: the SMO changes configuration without logging into the device.',
    'YANG.o': 'The language that describes what a device can do: every parameter, type and limit.',
    'YANG.p': 'The device advertises its YANG models and the SMO learns what it can configure there.',
    'RESTCONF.o': 'The same YANG model as NETCONF, reached over HTTP and JSON.',
    'RESTCONF.p': 'Lets web screens and scripts talk to the controller without a NETCONF client.',
    'M-plane.o': 'The Open Fronthaul management plane, between the O-DU or the SMO and the O-RU.',
    'M-plane.p': 'Configures and supervises the radio: straight from the SMO in the hybrid model, or through the O-DU in the hierarchical one.',
    'PNF.o': 'A network function on its own hardware, like an O-RU — the opposite of a virtualized function.',
    'PNF.p': 'When it powers up, it announces itself to the SMO with a pnfRegistration to join the inventory.',
    'TAPI.o': 'The ONF standard API for transport topology and connectivity.',
    'TAPI.p': 'Describes who connects to whom in a format any SMO understands.',
    'OAuth.o': 'The token-based authorization standard: whoever logs in gets a temporary credential.',
    'OAuth.p': 'Services check the token instead of storing passwords — in the SMO, Keycloak issues it and the controller validates it.',
    'Kafka.o': 'A message bus: producers publish to topics, consumers subscribe.',
    'Kafka.p': 'Decouples who generates events from who uses them — VES publishes, analytics reads whenever it wants.',
    'Keycloak.o': 'Open-source identity server: users, roles and single sign-on.',
    'Keycloak.p': 'Centralizes who may use the SMO and with which role.',
    'OpenDaylight.o': 'Open-source network controller from the Linux Foundation, written in Java.',
    'OpenDaylight.p': 'It is the base of the O-RAN SC SDN-R controller, which speaks NETCONF to the elements.',
  ...
  es: {
    ...
    'NETCONF.o': 'El protocolo de configuración de red del IETF: lee y escribe la configuración de un equipo en transacciones.',
    'NETCONF.p': 'Es lo que usan la O1 y el M-plane: el SMO cambia la configuración sin entrar en el equipo.',
    'YANG.o': 'El lenguaje que describe lo que sabe hacer un equipo: cada parámetro, tipo y límite.',
    'YANG.p': 'El equipo anuncia sus modelos YANG y el SMO pasa a saber qué puede configurar en él.',
    'RESTCONF.o': 'El mismo modelo YANG de NETCONF, accedido por HTTP y JSON.',
    'RESTCONF.p': 'Permite que pantallas web y scripts hablen con el controlador sin cliente NETCONF.',
    'M-plane.o': 'El plano de gestión del Open Fronthaul, entre la O-DU o el SMO y el O-RU.',
    'M-plane.p': 'Configura y supervisa la radio: directo desde el SMO en el modelo híbrido, o a través de la O-DU en el jerárquico.',
    'PNF.o': 'Una función de red en hardware propio, como un O-RU — lo contrario de una función virtualizada.',
    'PNF.p': 'Al encenderse, se anuncia al SMO con un pnfRegistration para entrar en el inventario.',
    'TAPI.o': 'La API estándar de topología y conectividad de transporte, de la ONF.',
    'TAPI.p': 'Describe quién se conecta con quién en un formato que cualquier SMO entiende.',
    'OAuth.o': 'El estándar de autorización por token: quien inicia sesión recibe una credencial temporal.',
    'OAuth.p': 'El servicio verifica el token en lugar de guardar contraseñas — en el SMO, Keycloak lo emite y el controlador lo valida.',
    'Kafka.o': 'Un bus de mensajes: los productores publican en tópicos y los consumidores se suscriben.',
    'Kafka.p': 'Desacopla a quien genera eventos de quien los usa — el VES publica y el análisis lee cuando quiere.',
    'Keycloak.o': 'Servidor de identidad de código abierto: usuarios, roles e inicio de sesión único.',
    'Keycloak.p': 'Centraliza quién puede usar el SMO y con qué rol.',
    'OpenDaylight.o': 'Controlador de red de código abierto de la Linux Foundation, escrito en Java.',
    'OpenDaylight.p': 'Es la base del controlador SDN-R del O-RAN SC, que habla NETCONF con los elementos.',
  ...
  fr: {
    ...
    'NETCONF.o': 'Le protocole de configuration réseau de l’IETF : lit et écrit la configuration d’un équipement par transactions.',
    'NETCONF.p': 'C’est ce qu’utilisent l’O1 et le M-plane : le SMO change la configuration sans se connecter à l’équipement.',
    'YANG.o': 'Le langage qui décrit ce qu’un équipement sait faire : chaque paramètre, type et limite.',
    'YANG.p': 'L’équipement annonce ses modèles YANG et le SMO sait alors ce qu’il peut y configurer.',
    'RESTCONF.o': 'Le même modèle YANG que NETCONF, accessible en HTTP et JSON.',
    'RESTCONF.p': 'Permet aux écrans web et aux scripts de parler au contrôleur sans client NETCONF.',
    'M-plane.o': 'Le plan de gestion de l’Open Fronthaul, entre l’O-DU ou le SMO et l’O-RU.',
    'M-plane.p': 'Configure et supervise la radio : directement depuis le SMO en modèle hybride, ou via l’O-DU en modèle hiérarchique.',
    'PNF.o': 'Une fonction réseau sur son propre matériel, comme un O-RU — l’inverse d’une fonction virtualisée.',
    'PNF.p': 'À l’allumage, elle s’annonce au SMO par un pnfRegistration pour entrer dans l’inventaire.',
    'TAPI.o': 'L’API standard de topologie et de connectivité de transport, de l’ONF.',
    'TAPI.p': 'Décrit qui est relié à qui dans un format que tout SMO comprend.',
    'OAuth.o': 'Le standard d’autorisation par jeton : qui se connecte reçoit un identifiant temporaire.',
    'OAuth.p': 'Le service vérifie le jeton au lieu de stocker des mots de passe — dans le SMO, Keycloak l’émet et le contrôleur le valide.',
    'Kafka.o': 'Un bus de messages : les producteurs publient dans des topics, les consommateurs s’abonnent.',
    'Kafka.p': 'Découple qui produit les événements de qui les utilise — le VES publie, l’analyse lit quand elle veut.',
    'Keycloak.o': 'Serveur d’identité open source : utilisateurs, rôles et authentification unique.',
    'Keycloak.p': 'Centralise qui peut utiliser le SMO et avec quel rôle.',
    'OpenDaylight.o': 'Contrôleur réseau open source de la Linux Foundation, écrit en Java.',
    'OpenDaylight.p': 'C’est la base du contrôleur SDN-R de l’O-RAN SC, qui parle NETCONF aux éléments.',
